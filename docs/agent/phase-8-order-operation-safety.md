...
7. MCP 提供订单详情、取消预检查、退票预览和支付状态查询，但只由服务端固定链调用。
...
工具层不接收 `userId` 参数，身份只来自 Agent 签名元数据和网关注入的用户上下文。返回订单详情时不包含证件号。

规划模型和最终汇总模型均不注册工具。意图校验完成后，服务端根据固定映射调用只读 MCP；阶段七购票执行工具只允许确认接口后的专用执行器调用。取消订单和退票写操作同样只能通过各自的确认执行器进入。
